...
export default async function PollaParticipantsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [membersRaw, invs, pts, grpCounts, spcCounts, ansCounts, totalQuestionsResult] = await Promise.all([
    db.select({
      id: pollaMembers.id,
    ...
    featGroups
      ? db.select({ userId: groupPredictions.userId, count: sql<number>`COUNT(${groupPredictions.id})` })
          .from(groupPredictions)
          .where(eq(groupPredictions.pollaId, polla.id))
          .groupBy(groupPredictions.userId)
      : Promise.resolve([]),

    featSpecials
      ? db.select({ userId: specialPredictions.userId, count: sql<number>`COUNT(${specialPredictions.id})` })
    ...
    featQuestions
      ? db.select({ userId: pollaAnswers.userId, count: sql<number>`COUNT(${pollaAnswers.id})` })
          .from(pollaAnswers)
          .where(eq(pollaAnswers.pollaId, polla.id))
          .groupBy(pollaAnswers.userId)
      : Promise.resolve([]),

    featQuestions
      ? db.select({ count: sql<number>`COUNT(${pollaQuestions.id})` })
          .from(pollaQuestions)
  ...
  const ansMap = Object.fromEntries(ansCounts.map(r => [r.userId, Number(r.count)]))
  ...
  return (
    <div className="space-y-6">
      ...
      <PollaParticipantsManager
        ...
        featureGroupPredictions={featGroups}
        featureSpecialPredictions={featSpecials}
